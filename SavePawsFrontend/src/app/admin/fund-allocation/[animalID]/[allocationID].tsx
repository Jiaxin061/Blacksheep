...
export default function AllocationDetailScreen() {
  ...
  const loadAllocation = useCallback(async () => {
    ...
    try {
      console.log(`📡 Fetching details for allocationID: ${allocationID}`);
      setError(null);
      const data = await fetchAllocationDetail(Number(allocationID));
      console.log("✅ Fetched Data:", data ? "Found" : "Null");

      if (!data) {
        setError("Allocation not found in database.");
      } else {
        setAllocation({
          ...data,
          animal: {
