...
const PostCard = ({ item, onDelete, onRestore }) => (
    <View style={styles.card}>
        ...
        {item.contentImage && (
            <Image
                source={{
                    uri: item.contentImage.startsWith('http') ? item.contentImage : `${ApiService.BASE_URL.replace('/api', '')}/uploads/${item.contentImage}`
                }}
...
const AdminCommunityManagementPage = () => {
    ...
    const loadPosts = async () => {
        ...
        const data = await ApiService.getAdminCommunityPosts(status);
        setPosts(data);
        setIsLoading(false);
    };
        ...
        Alert.alert(
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    style: "destructive",
                    onPress: async () => {
                        await ApiService.adminDeleteCommunityPost(post.id);
                        loadPosts();
                    }
                }
            ]
        ...
        Alert.alert(
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    text: "Confirm",
                    onPress: async () => {
                        await ApiService.restoreCommunityPost(post.id);
                        loadPosts();
                    }
                }
            ]
