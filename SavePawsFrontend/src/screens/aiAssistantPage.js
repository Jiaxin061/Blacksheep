...
import { askAI, getAIHistory, getFullAIHistory, clearAIHistory } from '../services/api';
...
const AIAssistantPage = () => {
    ...
    // Using simple MOCK ID for now, as in other parts, but try to get real from storage if possible
    ...
    const loadUserAndHistory = async () => {
        try {
            ...
            const uid = storedId ? parseInt(storedId) : 2;
    ...
    const loadHistory = async (uid) => {
        ...
        try {
            const history = await getAIHistory(uid);
            ...
            const fullHistory = await getFullAIHistory(uid);
    ...
    const handleSend = async () => {
        ...
        try {
            // Call API
            const response = await askAI(currentUserId, userMsg.text);
            const responseText = response.error ? "Sorry, I'm having trouble connecting to the server." : response.response;

            const aiMsg = { id: (Date.now() + 1).toString(), text: responseText, sender: 'ai' };
            setMessages(prev => [...prev, aiMsg]);
        } catch (e) {
            const errorMsg = { id: (Date.now() + 1).toString(), text: "Network Error. Please try again.", sender: 'ai' };
    ...
    const handleNewChat = async () => {
        ...
        await clearAIHistory(currentUserId);
        // Reset local UI state
        setMessages([INITIAL_GREETING]);
        // Update history modal data
        const fullHistory = await getFullAIHistory(currentUserId);
        setHistoryList(fullHistory || []);
    };
