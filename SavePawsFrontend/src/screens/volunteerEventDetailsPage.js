...
const VolunteerEventDetailsPage = () => {
            ...
            const loadEvent = async () => {
                if (params.id) {
                    try {
                        ...
                        const eventData = await ApiService.getEventDetails(params.id);
                        ...
                        setEvent({
                            ...
                            time: eventData.time_range || (
                                eventData.start_date && eventData.end_date ?
                                    `${new Date(eventData.start_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(eventData.end_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` :
                                    'Time TBD'
                            ),
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            const result = await ApiService.registerForEvent(userId, event.id);
