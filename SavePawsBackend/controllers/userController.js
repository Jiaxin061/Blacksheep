...
exports.getAllUsers = async (req, res, next) => {
  try {
    // Try to get from users table (main app) first, fallback to user table (donation portal)
    let users = await query(
      "SELECT id as userID, CONCAT(first_name, ' ', last_name) as name, email, 'user' as role FROM users ORDER BY first_name ASC, last_name ASC"
    ).catch(async () => {
      // Fallback to donation portal user table if users table doesn't work
      return await query(
...
exports.getUserProfile = async (req, res, next) => {
  try {
    ...
    // Try users table first (main app), fallback to user table (donation portal)
    let users = await query(
      "SELECT id as userID, CONCAT(first_name, ' ', last_name) as name, email, 'user' as role, created_at as createdAt FROM users WHERE id = ?",
      [userID]
    ).catch(async () => {
      return await query(
...
exports.getDonationImpactDetail = async (req, res, next) => {
  try {
    ...
    // Try joining with users table first, fallback to user table
    let donations = await query(
      ...
      FROM donation_transaction dt
      INNER JOIN users u ON dt.userID = u.id
      INNER JOIN animal_profile ap ON dt.animalID = ap.animalID
    ...
    const progressUpdates = await query(
      `SELECT 
        updateID,
        allocationID,
        title,
