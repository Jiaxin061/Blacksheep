...
exports.createAnimal = async (req, res, next) => {
  ...
  const { name, type, story, fundingGoal, status } = req.body;
  // Get adminID from authenticated admin (req.adminID or req.userID)
  const adminID = req.adminID || req.userID || req.body.adminID || 1;
...
exports.deleteAnimal = async (req, res, next) => {
  ...
  try {
    ...
    const animalData = await query(
      "SELECT photoURL FROM animal_profile WHERE animalID = ?",
      [id]
    );

    // Delete from database
    await query("DELETE FROM animal_profile WHERE animalID = ?", [id]);
