    ...
    try {
      ...
      if (response.success) {
        ...
        if (rescueTasks.length > 0) {
          const firstTaskWithLocation = rescueTasks.find(
            t => t.location_latitude && t.location_longitude
          );
          if (firstTaskWithLocation) {
            setMapRegion({
              latitude: parseFloat(firstTaskWithLocation.location_latitude),
              longitude: parseFloat(firstTaskWithLocation.location_longitude),
              latitudeDelta: 0.05,
              longitudeDelta: 0.05,
            });
          }
    ...
    >
      ...
      <View style={styles.taskContent}>
        ...
        <View style={styles.updateBox}>
          <Text style={styles.updateTitle}>User Update Details</Text>
          {item.assigned_to_user_id ? (
            <View>
              {item.update_text || item.update_image ? (
                <View>
                  {item.update_text && (
                    <View style={styles.updateTextContainer}>
                      <Text style={styles.updateLabel}>Update Text:</Text>
                      <Text style={styles.updateText}>{item.update_text}</Text>
                    </View>
                  )}
                  {item.update_image && (
                    <View style={styles.updateImageContainer}>
                      <Text style={styles.updateLabel}>Update Image:</Text>
                      <Image
                        source={{ uri: item.update_image }}
                  ...
                  {item.assigned_at && (
                    <Text style={styles.updateTimestamp}>
                      Last updated: {new Date(item.assigned_at).toLocaleString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
        ...
        {filteredReports
          ...
          .map((task) => (
            <Marker
              ...
              pinColor={getUrgencyColor(task.urgency_level)}
            >
              <View style={[styles.markerContainer, { borderColor: getUrgencyColor(task.urgency_level) }]}>
