...
import { useAdminRegistrationController } from '../_controller/AdminRegistrationController';
...
export default function AdminRegistrationManagementPage() {
    ...
    const { getPendingRegistrations, approveRegistration, rejectRegistration, loading, error } = useAdminRegistrationController();
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    text: 'Approve',
                    onPress: async () => {
                        const success = await approveRegistration(reg.registrationID);
                        if (success) {
    ...
    const handleConfirmReject = async () => {
        ...
        const success = await rejectRegistration(selectedReg.registrationID, rejectionReason);
        if (success) {
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity
            ...
            onPress={() => {
                router.push({
                    pathname: '/pages/adminRegistrationDetailsPage',
                    params: { registration: JSON.stringify(item) }
                });
            }}
        >
