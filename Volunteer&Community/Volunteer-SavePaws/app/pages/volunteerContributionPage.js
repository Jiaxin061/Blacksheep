    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', () => {
            const loadData = async () => {
                try {
                    const userId = MOCK_CURRENT_USER_ID;

                    // 1. Fetch Registered Events (Upcoming)
                    const myEvents = await VolunteerService.getUserEvents(userId);
                    ...
                    const mappedUpcoming = myEvents.map(e => ({
                        ...
                        title: e.title,
                        location: e.location,
                        time: e.time_range || 'Time TBD',
                        startDate: e.event_date,
                        image: e.image_url,
                        tag: e.tag_text || 'Upcoming',
                        tagColor: e.tag_color || Colors.primary,
                        volunteers: 0
                    }));
                    setUpcomingEvents(mappedUpcoming);

                    // 2. Fetch Contributions (Past History)
                    const myContributions = await VolunteerService.getContributions(userId);
                    ...
                    try {
                        const regDetails = await VolunteerService.getRegistrationDetails(userId);
