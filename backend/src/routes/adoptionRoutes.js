import express from 'express';
import { authenticateToken, isAdmin } from '../middleware/authMiddleware.js';
import {
  createRequest,
  getMyRequests,
  getAllRequests,
  getRequestById,
  updateStatus
} from '../controllers/adoptionController.js';

import {
  createUpdate,
  getMyUpdates,
  getUpdates,
  reviewUpdate
} from '../controllers/adoptionUpdateController.js';
import { upload } from '../middleware/upload.js';

const router = express.Router();

// UC016: Submit adoption request (Authenticated users)
router.post('/', authenticateToken, createRequest);

// Get authenticated user's requests
router.get('/my-requests', authenticateToken, getMyRequests);

// UC025: Submit adoption follow-up update (Authenticated users)
router.post('/updates', authenticateToken, upload.single('photo'), createUpdate);

// Get authenticated user's updates
router.get('/my-updates', authenticateToken, getMyUpdates);

// UC017: Get all adoption requests (Admin only, with optional status filter)
router.get('/', authenticateToken, isAdmin, getAllRequests);

// UC026: Get all adoption updates (Admin only)
router.get('/updates', authenticateToken, isAdmin, getUpdates);

// UC026: Review adoption update (Admin only)
router.patch('/updates/:id/review', authenticateToken, isAdmin, reviewUpdate);

// Get single adoption request by ID (Admin only)
router.get('/:id', authenticateToken, isAdmin, getRequestById);

// UC017: Update adoption request status (Admin only)
router.patch('/:id/status', authenticateToken, isAdmin, updateStatus);

export default router;


