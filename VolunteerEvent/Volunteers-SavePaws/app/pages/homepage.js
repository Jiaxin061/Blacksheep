...
// --- EMBEDDED MOCK REGISTRATION STATUS LOGIC (MUST MATCH volunteerRegistrationPage.js) ---
// MOCK: This simulates checking the static 'isUserRegistered' field in RegistrationController.java
// Import the shared state from the registration page
import { isUserRegisteredMock } from './volunteerRegistrationPage';

const mockRegistrationController = () => ({
  // Simulates calling RegistrationController.getRegistrationStatus(userId)
  getRegistrationStatus: async () => {
    // Simulate network latency for API call
    await new Promise(resolve => setTimeout(resolve, 300));
    return isUserRegisteredMock;
  }
});
// -----------------------------------------------------------------------------------------
...
const HomePage = () => {
  ...
  // NEW: State for showing a loading spinner while checking status
  const [isCheckingStatus, setIsCheckingStatus] = useState(false);

  // NEW: Handler for the Volunteer button press
  const handleVolunteerPress = async () => {
    setIsCheckingStatus(true);
    const isRegistered = await mockRegistrationController().getRegistrationStatus(123); // Use a mock UserID
    setIsCheckingStatus(false);

    if (isRegistered) {
      // If registered: Navigate to the Contribution Dashboard
      navigation.navigate('VolunteerContributionPage');
    } else {
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.servicesRow}>
            ...
            {/* 3. VOLUNTEER BUTTON (LOGIC INJECTED) */}
            <TouchableOpacity
              style={[styles.serviceCard, isCheckingStatus && { opacity: 0.7 }]}
              onPress={() => router.push('/pages/volunteerContributionPage')}
